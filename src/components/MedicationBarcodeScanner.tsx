...
interface Props {
  ...
  onCancel: () => void;
  onSwitchToOCR?: () => void; // <- from his version
}

export default function MedicationBarcodeScanner({
  ...
  const scanBarcode = async (imageUrl: string) => {
    ...
    try {
      addDebugLog('Creating Image element...');
      const img = new Image();
      img.src = imageUrl;
      ...
      addDebugLog(
        `Image loaded successfully: ${img.width}x${img.height} pixels`
      );
      ...
      if (width > maxDimension || height > maxDimension) {
        ...
        addDebugLog(
          `Scaling image down to: ${Math.round(width)}x${Math.round(height)}`
        );
      }
      ...
      const scaledImageUrl = canvas.toDataURL('image/jpeg', 0.95);
      const scaledImg = new Image();
      scaledImg.src = scaledImageUrl;

      await new Promise((resolve) => {
        scaledImg.onload = resolve;
      });

      addDebugLog('Image preprocessed, attempting barcode detection...');
      ...
      const orientedImages: Array<{ angle: number; img: HTMLImageElement }> = [];
      orientedImages.push({ angle: 0, img: scaledImg });
      ...
      for (const angle of angles) {
        ...
        if (angle === 90 || angle === 270) {
          rc.width = scaledImg.height;
          rc.height = scaledImg.width;
        } else {
          rc.width = scaledImg.width;
          rc.height = scaledImg.height;
        }
        ...
        const rotatedUrl = rc.toDataURL('image/jpeg', 0.95);
        const rotatedImg = new Image();
        rotatedImg.src = rotatedUrl;
        await new Promise((resolve) => {
          rotatedImg.onload = resolve;
        });
        orientedImages.push({ angle, img: rotatedImg });
      }

      const readers = [
        { name: 'MultiFormat', make: () => new BrowserMultiFormatReader(hints as any) },
        { name: 'UPC/EAN', make: () => new BrowserBarcodeReader() },
      ];

      for (const { name, make } of readers) {
        for (const { angle, img: tryImg } of orientedImages) {
          try {
            addDebugLog(`Trying ${name} reader at ${angle}° ...`);
            const reader = make();
            // @ts-ignore: set hints if available
            if ((reader as any).hints === undefined && name === 'MultiFormat') {
              (reader as any).hints = hints;
            }

            const result: Result = await reader.decodeFromImageElement(tryImg);
            const barcodeData = result.getText();
            const barcodeFormat = result.getBarcodeFormat();

            addDebugLog(`✓ BARCODE DETECTED with ${name} at ${angle}°!`);
            addDebugLog(`Format: ${BarcodeFormat[barcodeFormat]}`);
            addDebugLog(`Raw data: "${barcodeData}"`);
            addDebugLog(`Length: ${barcodeData.length} characters`);
            ...
            const isRetailFormat = [
              BarcodeFormat.UPC_A,
              BarcodeFormat.UPC_E,
              BarcodeFormat.EAN_13,
              BarcodeFormat.EAN_8,
            ].includes(barcodeFormat);

            if (fdaMedication) {
              addDebugLog(
                `✓ FDA medication found: ${
                  fdaMedication.brand_name || fdaMedication.generic_name
                }`
              );
              setLoading(false);
              return;
              ...
              addDebugLog(
                'NDC lookup failed. Detected retail barcode format (UPC/EAN), attempting product lookup...'
              );
              const upcProduct = await lookupProductByUPC(barcodeData);

              if (upcProduct) {
                addDebugLog(
                  `✓ UPC/EAN product found: ${
                    upcProduct.title || upcProduct.brand || 'Unnamed'
                  }`
                );
                ...
                setError('');
                addDebugLog(
                  `Passing scanned product to parent component...`
                );
                ...
                addDebugLog(
                  '✗ UPC/EAN product lookup returned no data.'
                );
                setError(
                  'Product found but not in medication database. You can continue with manual entry.'
                );
              }
            } else {
              addDebugLog(
                '✗ No FDA medication found and barcode is not retail format (UPC/EAN).'
              );
              setError(
                'Medication not found in FDA database. This may be a non-NDC barcode.'
              );
            }
            ...
            addDebugLog(
              `✗ ${name} at ${angle}° failed: ${err.message || err}`
            );
            continue;
          }
        }
      ...
      throw new Error('All barcode readers failed to detect the code');
    } catch (err: any) {
      addDebugLog('✗✗✗ ALL BARCODE DETECTION FAILED');
      addDebugLog(`Error: ${err.message || err}`);
      addDebugLog(
        'Suggestions: Try a clearer image, better lighting, or crop closer to barcode'
      );
      console.error('Full error:', err);
      setError(
        'No barcode detected in image. Please try another image with a clear barcode.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const convertToNDCFormat = (code: string): string[] => {
    ...
    if (noLeadingZeros.length >= 8) {
      formats.push(
        `${noLeadingZeros.slice(0, 4)}-${noLeadingZeros.slice(4, 8)}`
      );
      formats.push(
        `${noLeadingZeros.slice(0, 5)}-${noLeadingZeros.slice(5, 8)}`
      );
    }

    const uniqueFormats = [...new Set(formats)];
    addDebugLog(
      `Generated ${uniqueFormats.length} NDC format(s): ${uniqueFormats.join(
        ', '
      )}`
    );
  ...
  const lookupMedication = async (code: string) => {
    ...
    addDebugLog(
      `Starting FDA API lookup with ${possibleFormats.length} format(s)...`
    );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.results && data.results.length > 0) {
            addDebugLog(
              `✓ SUCCESS! Found medication: ${
                data.results[0].brand_name || data.results[0].generic_name
              }`
            );
    ...
    addDebugLog(
      '✗✗✗ ALL FORMATS FAILED - Medication not found in FDA database'
    );
    setError(
  ...
  const handleUseThisMedication = () => {
    ...
    const scannedMed: ScannedMedication = {
      ...
      dosage:
        medication.active_ingredients?.[0]?.strength || 'See package',
      frequency: 'As directed',
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      {barcode && (
                        <div className="bg-white p-3 rounded-lg border border-red-200">
                          <div className="flex items-center justify-between">
                            ...
                            {barcodeFormatStr && (
                              <span className="text-xs px-2 py-1 rounded-md bg-slate-100 border border-slate-200 text-slate-600 font-medium">
                                {barcodeFormatStr}
                              </span>
                            )}
                          </div>
                          <p className="mt-2 text-xs text-slate-500">
                            The format {barcodeFormatStr || 'Unknown'} may be a
                            retail UPC and not an FDA NDC.
                          </p>
                        </div>
                      )}
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 space-y-2">
                        ...
                        <p className="text-amber-700 text-xs mt-2">
                          If not found after extensive search, try scanning a
                          different barcode on the package or use OCR scan
                          instead.
                        </p>
                      </div>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                      <p className="text-xs text-amber-800">
                        <strong className="font-semibold">Note:</strong> This
                        information is retrieved from the FDA database for
                        reference. Please verify all details and complete
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                  ...
                  <ul className="space-y-3 text-sm text-slate-600">
                    ...
                    <li className="flex items-start gap-2">
                      <span className="text-emerald-600 mt-0.5">✓</span>
                      <span>
                        Use good lighting to avoid shadows on the barcode
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-emerald-600 mt-0.5">✓</span>
                      <span>
                        Hold the camera steady and close enough to read the
                        barcode
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-emerald-600 mt-0.5">✓</span>
                      <span>
                        Works with NDC (National Drug Code) barcodes on
                        medication bottles
                      </span>
                    </li>
                  </ul>
