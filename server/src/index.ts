...
const allowedOrigins = [
  "http://localhost:3000",
  "http://localhost:3001",
  "https://dareyno4.github.io", // GitHub pages root
...
// Mock email endpoint (HIPAA-safe: logs only, does not send)
app.post("/api/send-email", async (req: any, res: any) => {
  ...
  try {
    console.log("---------------------------------------------------");
    console.log("📧 EMAIL DELIVERY DISABLED (HIPAA Compliance)");
    console.log(`To: ${to}`);
    console.log(`Subject: ${subject}`);
    console.log("--- Content Preview ---");
    console.log(html.substring(0, 500) + "...");
    console.log("---------------------------------------------------");
    console.log("ℹ️  To enable HIPAA-compliant email delivery:");
    console.log("   1. Set up SendGrid with BAA");
    console.log("   2. Add SENDGRID_API_KEY to .env");
    console.log("   3. Update this endpoint to use SendGrid");
    console.log("---------------------------------------------------");
    ...
    return res.json({
      ...
      mock: true,
      message:
        "Email delivery is currently disabled. Contact your administrator to enable HIPAA-compliant email service.",
    });
  } catch (error: any) {
...
app.post("/api/activate-user", async (req: any, res: any) => {
  try {
    ...
    if (!activationCode || !password) {
      return res
        .status(400)
        .json({ error: "Missing activation code or password" });
    }
    ...
    if (new Date(invitation.expires_at) < new Date()) {
      ...
      return res
        .status(400)
        .json({ error: "Activation code has expired" });
    }
    ...
    const { data: userData, error: userError } = await supabaseAdmin
      ...
      .insert({
        ...
        tenant_id: invitation.tenant_id,
        phone_number:
          additionalData?.phone_number || invitation.phone_number,
...
app.post("/api/activate-tenant", async (req: any, res: any) => {
  try {
    ...
    if (!tenant.baa_signed) {
      return res
        .status(400)
        .json({ error: "BAA must be signed before activation" });
    }
    ...
    if (metadata.activation_used) {
      return res
        .status(400)
        .json({ error: "Activation code has already been used" });
    }
...
app.listen(process.env.PORT || 8080, () => {
  ...
  console.log("⚠️  Email delivery disabled for HIPAA compliance");
  console.log(
    "ℹ️  To enable email: Set up SendGrid with BAA and add SENDGRID_API_KEY"
  );
});
